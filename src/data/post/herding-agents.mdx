...
Work is scoped to small meaningful changes that make progress on their own. Standard engineering stuff. Sometimes I ask for changes with loose definitions and through the design discussion I am able to split it up into smaller, well-defined, manageable pieces. In practice, these chunks of work take between 20m and 2hrs complete. It's not a target I aim for, just a consequence of tightly scoped work.
...
8. **History rewrite.** A subagent opens a new PR with a fresh set of ordered and titled commits for me to review. This hides the internal synchronization that don't belong in my review. The new state is statically evaluated to match, and the draft PR is closed.
